...
# Signature will always contain 3 numbers; n will always be a non-negative number; if n == 0, then return an empty array (except in C return NULL) and be ready 
...
def tribonacci(signature, n):
    # Declare a list to hold the sequence and an index variable to use for the list construction.
    sequence = []
    return_sequence = []
    index = 3
    sequence.append(signature[0])
    sequence.append(signature[1])
    sequence.append(signature[2])
    temp = 0
    
    # Construct the sequence to n values.
    while index < n:
        temp = sequence[index - 3] + sequence[index - 2] + sequence[index - 1]
        sequence.append(temp)
        index += 1
    
    # Construct the return sequence of n values from the constructed Tribonacci Sequence.
    for num in range(0,n):
        return_sequence.append(sequence[num])
    
    # Return the return sequence.
